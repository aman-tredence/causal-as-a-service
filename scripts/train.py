...
class Training:
    def __init__(self):
            self.version = 1
            self.binary_methods = ["backdoor.propensity_score_matching",
                                   "backdoor.propensity_score_weighting",
                                   "backdoor.propensity_score_stratification"]
    def get_latest_version(self):
        models_available = os.listdir(self.model_output_path)
        if len(models_available)>0:
            latest_available_version = int(pd.Series(models_available).apply(lambda x:get_version(x)).max())
    ...
    def refutation_check(self, model, identified_estimand, causal_estimate):
            if self.refutation_method:
                ...
                refutation_result = model.refute_estimate(identified_estimand,
                                                          causal_estimate,
                                                          method_name = self.refutation_method)
    ...
    def causal_analysis(self):
            ...
            if self.estimate_method in self.binary_methods:
                for treatment in self.treatment_vars:
                    ...
                    train_df[treatment] = train_df[treatment].apply(lambda x: 1 if x > thresh else 0)
            ...
            if self.estimate_method == 'backdoor.linear_regression':
                #causal Estimate
                causal_model,df_coeffs, causal_estimate = self.causal_estimate_linear(model, identified_estimand)         
                print("causal Estimate: ",causal_estimate.value)
            elif self.estimate_method == 'backdoor.generalized_linear_model':
                #causal Estimate
                causal_model,df_coeffs, causal_estimate = self.causal_estimate_logistic(model, identified_estimand)         
                print("causal Estimate: ",causal_estimate.value)
            #Refutatoin Check 
            new_effect, p_values = self.refutation_check(model, identified_estimand, causal_estimate)
            print('Training Completed for model')
            #Saving the model and model artifacts
            if self.save_status:
                self.save_model_files(causal_model, identified_estimand, df_coeffs, causal_estimate, new_effect, p_values)
    ...
    def save_model_files(self, Causal_model, identified_estimand, df_coeffs, Causal_estimate, new_effect, p_value):
            # Saving Model--------------------------------------------------------------------------------
            ...
            self.model_name = self.model_output_path+f'{self.target}_Causal_model_v{self.version}.pkl'
            with open(self.model_name, 'wb') as f:
                pickle.dump(Causal_model, f)
             # Saving Model Artifacts----------------------------------------------------------------------
            print('Saving Model Artifacts...')
            artifacts = pd.DataFrame(columns = ['Target', 'TreatmentVariables', 'TableName', 'DAG', 'ModelName', 'CausalEstimate', 
                                                'RefutationNewEstimate', 'p_value', 'EstimationMethod', 'Version', 'ModelCreationDate',
                                                'IdentifiedestimandFile', 'ModelURL'])            
            artifacts_dict = {
                'Target' : self.target,
            ...
            artifacts_file = self.data_output_path+f'identified_estimand_v{self.version}.pkl'
...
# Logic starts here
with open('C:\\Users\\pushpesh.pallav\\Downloads\\OneDrive_2023-12-11\\Causal as a Service Model\\Causal as a Service\\data\input\\train_config_linear.json') as tc:
